...
# --- ÇELİK KASA (VERİTABANI) AYARLARI ---
conn = sqlite3.connect('togg_sarj_v3.db', check_same_thread=False)
c = conn.cursor()
...
df_kontrol = pd.read_sql_query("SELECT COUNT(*) as sayi FROM sarj_kayitlari", conn)
kutu_acik_mi = True if df_kontrol['sayi'][0] == 0 else False

st.sidebar.markdown("### ⚙️ Ayarlar")
with st.sidebar.expander("📥 İlk Kurulum (Dosya Yükle)", expanded=kutu_acik_mi):
    st.write("Verilerinizi tek seferlik yükleyin.")
    uploaded_file = st.file_uploader("İndirdiğiniz 'SARJ.csv' dosyasını yükleyin", type=["csv"])

    if uploaded_file is not None:
        try:
            # Çift sütun hatalarını yok eden garantili okuma yöntemi
            df_okunan = pd.read_csv(uploaded_file, sep=None, engine='python')
            df_okunan.columns = [str(col).strip().replace('\ufeff', '').replace('ï»¿', '').upper() for col in df_okunan.columns]
            
            c.execute("DELETE FROM sarj_kayitlari")
            
            for row in df_okunan.to_dict('records'):
                ...
                st_tarih = get_val(['TARİH', 'TARIH'], date.today().strftime("%d.%m.%Y")).replace('/', '.').replace('-', '.')
                s_tipi = get_val(['ŞARJ TİPİ', 'SARJ TIPI', 'ŞARJ TIPI', 'SARJ TİPİ'], "AC")
                sfirma = get_val(['ALINAN ŞARJ FİRMASI', 'ALINAN SARJ FIRMASI', 'FİRMA'], "Bilinmiyor")
                
                skw_str = get_val(['FATURADA ALINAN KW', 'ALINAN KW (%1=0,885 KW)', 'ALINAN KW'], "0")
                skw = float(skw_str.replace('.', '').replace(',', '.')) if skw_str.lower() != "nan" and skw_str != "" else 0.0
                
                stutar_str = get_val(['ÖDENEN ÜCRET', 'ODENEN UCRET', 'TUTAR'], "0")
                ...
                skm_str = get_val(['ARAÇ KM', 'ARAC KM', 'KM'], "0")
                try:
            ...
            conn.commit()
            st.rerun() # Sayfayı yeniler, kutuyu sonsuza dek kapatır
        except Exception as e:
            st.error(f"Dosya okunamadı. Lütfen CSV formatında indirdiğinizden emin olun. Detay: {e}")
